import pygame
import os
from jogo_perguntas.config import CAMINHO_IMAGEM_FUNDO_PERSONALIZADA
from config import LARGURA_JANELA, ALTURA_JANELA # Adicionado LARGURA_JANELA, ALTURA_JANELA caso for necessário
...
class GerenciadorInterface:
    ...
    def _carregar_imagem_fundo(self):
        if CAMINHO_IMAGEM_FUNDO_PERSONALIZADA and os.path.exists(CAMINHO_IMAGEM_FUNDO_PERSONALIZADA):
            try:
                ...
                self.imagem_fundo = pygame.transform.scale(imagem_original, (self.LARGURA_JANELA, self.ALTURA_JANELA))
    ...
    def get_fonte_ui(self, tipo_tamanho_str):
        return Tema.get_fonte(tipo_tamanho_str, self.ALTURA_JANELA)
    ...
    def desenhar_caixa_pergunta(self, tela, texto_pergunta):
        ...
        altura_caixa = self.ALTURA_JANELA * 0.25
        rect_caixa = pygame.Rect(margem_externa_caixa, margem_externa_caixa * 0.8, 
                                 self.LARGURA_JANELA - 2 * margem_externa_caixa, altura_caixa)
    ...
    def desenhar_balao_dica(self, tela, texto_dica, rect_botao_dica_referencia):
        ...
        padding_balao = 10
        largura_max_texto_dica = self.LARGURA_JANELA * 0.55
        ...
        x_balao = max(10, min(x_balao, self.LARGURA_JANELA - largura_total_balao - 10)) 
    ...
    def desenhar_popup_confirmacao(self, tela, lista_botoes_popup, texto_principal_popup="Confirmar?"):
        overlay_surface = pygame.Surface((self.LARGURA_JANELA, self.ALTURA_JANELA), pygame.SRCALPHA)
        ...
        largura_caixa_popup = max(max_largura_texto_popup + 2 * padding_caixa_popup, 
                                  ...
                                  self.LARGURA_JANELA * 0.6)
        altura_caixa_popup = altura_total_textos_popup + altura_botao_no_popup + espaco_entre_botoes_horizontal + 2 * padding_caixa_popup
        x_caixa_popup = (self.LARGURA_JANELA - largura_caixa_popup) // 2
        y_caixa_popup = (self.ALTURA_JANELA - altura_caixa_popup) // 2
    ...
    def desenhar_tela_final(self, tela, pontuacao_final, lista_botoes_tela_final, texto_info_resposta_correta=None):
        ...
        if texto_info_resposta_correta:
            ...
            linha_atual_str = ""
            largura_max_info_final = self.LARGURA_JANELA * 0.75
        ...
        largura_max_conteudo_interno = max(larguras_conteudo) if larguras_conteudo else self.LARGURA_JANELA * 0.5
        ...
        largura_caixa_final = max(largura_max_conteudo_interno + 2 * padding_caixa_final, self.LARGURA_JANELA * 0.6)
        ...
        x_caixa_final = (self.LARGURA_JANELA - largura_caixa_final) // 2
        y_caixa_final = (self.ALTURA_JANELA - altura_caixa_final) // 2
    ...
    def desenhar_notificacao_flutuante(self, tela):
        ...
        x_pos_notif = (self.LARGURA_JANELA - largura_notif) // 2
        y_pos_notif = 20 
    ...
    def desenhar_info_pontuacao(self, tela, valor_pontuacao):
        ...
        nova_pos_x = 270
        nova_pos_y = 10
        tela.blit(surface_texto_pontuacao, (self.LARGURA_JANELA -  nova_pos_x, nova_pos_y))